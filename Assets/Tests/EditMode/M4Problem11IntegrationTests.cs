...
public class M4Problem11IntegrationTests
{
    #region MARK - Recursos do problema 11
    ...
    [Test]
    public void PrefabDeA11DependeDiretamenteDoFbxCanonicoENaoDoModeloLegado()
    {
        GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPathA11);
        Assert.That(prefab, Is.Not.Null);

        string[] dependencias = AssetDatabase.GetDependencies(PrefabPathA11, true);
    ...
    [Test]
    public void PrefabDeA11ContemDisplayDinamicoGerenciadorVisualEControladorDeLeds()
    {
        GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPathA11);
    ...
    [Test]
    public void PrefabDeA11UsaMaterialEmissivoNoRendererDeLeds()
    {
        GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPathA11);
    ...
    [Test]
    public void SelecaoA11ResolveM4SmartTesteComDisplayELedsNoFluxoAr()
    {
        ...
        var objetoSelecao = new GameObject("Selecao A11 Teste AR");
        ...
        try
        {
            ...
            tipoSelecao.GetMethod("SelecionarAlertaOficial").Invoke(selecao, new object[] { "A11" });
            var prefabResolvido = exibidor.GetType().BaseType
                .GetProperty("PrefabDoModelo")
                .GetValue(exibidor) as GameObject;

            Assert.That(AssetDatabase.GetAssetPath(prefabResolvido), Is.EqualTo(PrefabPathA11));
            Assert.That(AssetDatabase.GetAssetPath(prefabResolvido), Is.Not.EqualTo(PrefabPathA8));
            Assert.That(AssetDatabase.GetDependencies(PrefabPathA11), Does.Contain(ModelPath));
    ...
    [Test]
    public void SelecaoA11ResolveOMesmoPrefabNoFluxoArENoVisualizador3D()
    {
        EditorSceneManager.OpenScene(ScenePath, OpenSceneMode.Single);
        ...
        var objetoSelecao = new GameObject("Selecao A11 Teste Comparativo");
        Component selecao = objetoSelecao.AddComponent(tipoSelecao);
        PropertyInfo instancia = tipoSelecao.GetProperty("Instance", BindingFlags.Public | BindingFlags.Static);
        ...
        try
        {
            ...
            objetoVisualizador = new GameObject("Visualizador3D Teste A11");
            ...
            Assert.That(AssetDatabase.GetAssetPath(prefabResolvidoAr), Is.EqualTo(PrefabPathA11));
            Assert.That(AssetDatabase.GetAssetPath(prefabResolvidoVisualizador), Is.EqualTo(PrefabPathA11));
    ...
    [Test]
    public void ModeloDeA11DesabilitaAnimacaoMecanicaLegada()
    {
        EditorSceneManager.OpenScene(ScenePath, OpenSceneMode.Single);
        MonoBehaviour exibidor = Object.FindObjectsByType<MonoBehaviour>(FindObjectsSortMode.None)
            .First(component => component.GetType().Name == "PlaceOnPlane_Adaptado");
        Type tipoSelecao = Type.GetType("ProblemaSelecionadoAR, Sense.Runtime");
        var objetoSelecao = new GameObject("Selecao A11 Teste Animacao");
        Component selecao = objetoSelecao.AddComponent(tipoSelecao);
        PropertyInfo instancia = tipoSelecao.GetProperty("Instance", BindingFlags.Public | BindingFlags.Static);
