...
public class M4Problem13IntegrationTests
{
    #region MARK - Recursos do problema 13

    private const string ModelPath = "Assets/Prefab/Teste/M4SMARTTeste.fbx";
    private const string PrefabPathA13 = "Assets/Resources/M4Problem13/M4SMARTTesteProblema13.prefab";
    ...
    [Test]
    public void PrefabDeA13DependeDiretamenteDoFbxCanonicoENaoDoModeloLegado()
    {
        GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPathA13);
        Assert.That(prefab, Is.Not.Null);

        string[] dependencias = AssetDatabase.GetDependencies(PrefabPathA13, true);
    ...
    [Test]
    public void PrefabDeA13ContemDisplayDinamicoGerenciadorVisualEControladorDeLeds()
    {
        GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPathA13);
    ...
    [Test]
    public void PrefabDeA13UsaMaterialEmissivoNoRendererDeLeds()
    {
        GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPathA13);
    ...
    [Test]
    public void SelecaoA13ResolveM4SmartTesteComDisplayELedsNoFluxoAr()
    {
        ...
        try
        {
            ...
            tipoSelecao.GetMethod("SelecionarAlertaOficial").Invoke(selecao, new object[] { "A13" });
            var prefabResolvido = exibidor.GetType().BaseType
                .GetProperty("PrefabDoModelo")
                .GetValue(exibidor) as GameObject;

            Assert.That(AssetDatabase.GetAssetPath(prefabResolvido), Is.EqualTo(PrefabPathA13));
            ...
            Assert.That(AssetDatabase.GetDependencies(PrefabPathA13), Does.Contain(ModelPath));
    ...
    [Test]
    public void SelecaoA13ResolveOMesmoPrefabNoFluxoArENoVisualizador3D()
    {
        EditorSceneManager.OpenScene(ScenePath, OpenSceneMode.Single);
        ...
        try
        {
            ...
            objetoVisualizador = new GameObject("Visualizador3D Teste A13");
            ...
            Assert.That(AssetDatabase.GetAssetPath(prefabResolvidoAr), Is.EqualTo(PrefabPathA13));
            Assert.That(AssetDatabase.GetAssetPath(prefabResolvidoVisualizador), Is.EqualTo(PrefabPathA13));
    ...
    [Test]
    public void ModeloDeA13DesabilitaAnimacaoMecanicaLegada()
    {
        EditorSceneManager.OpenScene(ScenePath, OpenSceneMode.Single);
        MonoBehaviour exibidor = Object.FindObjectsByType<MonoBehaviour>(FindObjectsSortMode.None)
            .First(component => component.GetType().Name == "PlaceOnPlane_Adaptado");
