...
public class M4Problem14IntegrationTests
{
    #region MARK - Recursos do problema 14

    private const string ModelPath = "Assets/Prefab/Teste/M4SMARTTeste.fbx";
    private const string PrefabPathA14 = "Assets/Resources/M4Problem14/M4SMARTTesteProblema14.prefab";
    ...
    [Test]
    public void PrefabDeA14DependeDiretamenteDoFbxCanonicoENaoDoModeloLegado()
    {
        GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPathA14);
        Assert.That(prefab, Is.Not.Null);

        string[] dependencias = AssetDatabase.GetDependencies(PrefabPathA14, true);
    ...
    [Test]
    public void PrefabDeA14ContemDisplayDinamicoGerenciadorVisualEControladorDeLeds()
    {
        GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPathA14);
    ...
    [Test]
    public void PrefabDeA14UsaMaterialEmissivoNoRendererDeLeds()
    {
        GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPathA14);
    ...
    [Test]
    public void SelecaoA14ResolveM4SmartTesteComDisplayELedsNoFluxoAr()
    {
        ...
        try
        {
            ...
            tipoSelecao.GetMethod("SelecionarAlertaOficial").Invoke(selecao, new object[] { "A14" });
            var prefabResolvido = exibidor.GetType().BaseType
                .GetProperty("PrefabDoModelo")
                .GetValue(exibidor) as GameObject;

            Assert.That(AssetDatabase.GetAssetPath(prefabResolvido), Is.EqualTo(PrefabPathA14));
            ...
            Assert.That(AssetDatabase.GetDependencies(PrefabPathA14), Does.Contain(ModelPath));
    ...
    [Test]
    public void SelecaoA14ResolveOMesmoPrefabNoFluxoArENoVisualizador3D()
    {
        EditorSceneManager.OpenScene(ScenePath, OpenSceneMode.Single);
        ...
        try
        {
            ...
            objetoVisualizador = new GameObject("Visualizador3D Teste A14");
            ...
            Assert.That(AssetDatabase.GetAssetPath(prefabResolvidoAr), Is.EqualTo(PrefabPathA14));
            Assert.That(AssetDatabase.GetAssetPath(prefabResolvidoVisualizador), Is.EqualTo(PrefabPathA14));
    ...
    [Test]
    public void ModeloDeA14DesabilitaAnimacaoMecanicaLegada()
    {
        EditorSceneManager.OpenScene(ScenePath, OpenSceneMode.Single);
        MonoBehaviour exibidor = Object.FindObjectsByType<MonoBehaviour>(FindObjectsSortMode.None)
            .First(component => component.GetType().Name == "PlaceOnPlane_Adaptado");
    ...
    [Test]
    public void FerramentaDeCriacaoDoPrefabDeA14EhIdempotente()
    {
        Type criador = Type.GetType("CriadorDePrefabDeAlertaA14, Assembly-CSharp-Editor");
        Assert.That(criador, Is.Not.Null);

        Assert.That(criador.GetField("PrefabDestinoA14").GetRawConstantValue(), Is.EqualTo(PrefabPathA14));
        Assert.That(criador.GetField("PrefabOrigemA8").GetRawConstantValue(), Is.EqualTo(PrefabPathA8));

        criador.GetMethod("CriarOuAtualizarPrefabA14").Invoke(null, null);

        Assert.That(AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPathA14), Is.Not.Null);
    }
